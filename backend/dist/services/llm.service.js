...
const ollama = new ollama_1.Ollama({ host: process.env.OLLAMA_BASE_URL || 'http://localhost:11434' });
console.log('[LLMService] OLLAMA_BASE_URL:', process.env.OLLAMA_BASE_URL || 'http://localhost:11434');
const CHAT_MODEL = 'llama3';
const EMBED_MODEL = 'nomic-embed-text';
class LLMService {
    static answerCache = new Map();
    static workspaceContextCache = new Map();
    static stopWords = new Set(['the', 'a', 'an', 'and', 'or', 'of', 'to', 'in', 'on', 'for', 'with', 'is', 'are', 'was', 'were', 'be', 'this', 'that', 'these', 'those', 'what', 'which', 'how', 'why', 'who', 'when', 'where', 'do', 'does', 'did', 'can', 'could', 'should', 'would', 'from', 'about', 'your', 'their', 'our', 'my', 'it', 'its', 'as', 'by', 'at', 'up', 'down', 'into', 'onto']);
    static storeWorkspaceContext(workspaceId, text, metadata) {
        if (!text || !text.trim())
            return;
        const entry = {
            text: text.trim(),
    ...
    static async getWorkspaceContext(workspaceId) {
        const cached = LLMService.workspaceContextCache.get(workspaceId);
        if (cached && cached.length > 0) {
            return cached.map(item => item.text).join('\n\n');
        }
        try {
            const docs = await Document_1.DocumentModel.find({ workspaceId, status: { $in: ['Processing', 'GeneratingEmbeddings', 'Completed'] } })
                .sort({ updatedAt: -1 })
                .limit(3)
                .select('originalUrl filename mimeType status');
            for (const doc of docs) {
                try {
                    const text = await textExtractor_service_1.TextExtractor.extract(doc.originalUrl, doc.mimeType || '');
                    if (text && text.trim()) {
    ...
    static isGeneralChatQuestion(question) {
    ...
    static async processAndStoreChunks(workspaceId, chunks) {
        const vectors = [];
        ...
        await vector_service_1.VectorService.upsertVectors(workspaceId, vectors);
    }
    static async generateAnswer(workspaceId, question) {
        ...
        if (quickContext) {
            ...
            try {
                const response = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Ollama chat timeout')), 10000)),
                ]);
                const result = {
        ...
        try {
            matches = await Promise.race([
                vector_service_1.VectorService.queryVectors(workspaceId, queryVector, 3),
        ...
        const result = {
            answer: response.message.content,
            citations: matches.map((m) => m.metadata),
        };
